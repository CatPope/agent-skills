...
**`-Force` 는 링크만 교체합니다. 실제 폴더는 지우지 않습니다** — 그 안에 있는 것이
유일본일 수 있기 때문입니다. 같은 이름의 실제 폴더가 있으면 설치기는 그 항목을
**실패로 보고하고 exit 1** 로 끝냅니다. 내용을 확인해 옮기거나 지운 뒤 다시 실행하세요.

설치기는 `mklink` 를 부른 것으로 성공을 세지 않고, **실제로 정션이 됐는지 확인한 뒤**
셉니다. 링크 수가 맞지 않으면 어느 경로가 왜 실패했는지 함께 출력합니다.
...
`implementation-conduct` 만 `audience: shared` 다. 나머지 `_core` 는 관리자용이다 —
구현 수칙은 **"누가"가 아니라 "무엇을 하는가"에 붙는** 규칙이라, 워크플로우에 따라
구현자가 바뀌어도 그대로 적용된다.
